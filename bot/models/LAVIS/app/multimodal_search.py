...
@st.cache(
    hash_funcs={torch.nn.parameter.Parameter: lambda parameter: parameter.data.detach().cpu().numpy()},
...
def app():
    ...
    num_display = st.sidebar.selectbox("Number of images:", values, index=default_layer_num)
    ...
    st.markdown("<h1 style='text-align: center;'>Multimodal Search</h1>", unsafe_allow_html=True)

    # === event ===
    vis_processor = load_processor("blip_image_eval").build(image_size=384)
    text_processor = load_processor("blip_caption")

    user_question = st.text_input("Search query", "A dog running on the grass.", help="Type something to search.")
    ...
    with torch.no_grad():
        text_feature = feature_extractor.extract_features(sample, mode="text").text_embeds_proj[0, 0]
    ...
    for i in range(num_batches):
        ...
        gradcam, itm_output = compute_gradcam_batch(itm_model, images, queries_batch, queries_tok_batch)
...
def compute_gradcam_batch(model, visual_input, text_input, tokenized_text, block_num=6):
    ...
    with torch.no_grad():
        ...
        gradcam = gradcam[:, 1 : token_length + 1, :].sum(dim=1, keepdim=True) / token_length
