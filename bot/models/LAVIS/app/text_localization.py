...
def app():
    ...
    layer_num = st.sidebar.selectbox("Layer number", values, index=default_layer_num) - 1

    st.markdown("<h1 style='text-align: center;'>Text Localization</h1>", unsafe_allow_html=True)
    ...
    query = st.text_input("Try a different input.", "A girl playing with her dog on the beach.")
        ...
        for _ in range(num_rows):
            with st.container():
                for col in st.columns(int(num_cols)):
                    ...
                    new_title = '<p style="text-align: center; font-size: 25px;">{}</p>'.format(word)
